...
#Creación del procedimiento con SET
...
BEGIN
    SET maximo := (SELECT MAX(precio) FROM productos WHERE gama = gam GROUP BY gama);
    SET minimo := (SELECT MIN(precio) FROM productos WHERE gama = gam GROUP BY gama);
    SET media := (SELECT AVG(precio) FROM productos WHERE gama = gam GROUP BY gama);
END //
DELIMITER ;
